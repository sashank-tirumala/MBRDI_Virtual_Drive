...
class Road:
    def __init__(self,road,scale=1):
    ...
    def get_lane_data(self,s):
        ...
        for lane_section in self.lane_sections:
            if(i+1 == len(self.lane_sections)):
                if(s < self.length):
                    res = {k: [val / self.scale for val in v] for k, v in lane_section.width(s).items()}
                    return res
                else:
                    return False
            if s > lane_section.s and s < self.lane_sections[i+1].s:
                res = {k: [val/self.scale for val in v] for k, v in lane_section.width(s).iteritems()}
                return res
            i += 1
        return None
    ...
    def get_elevation(self,s):
        ...
        return (self.elev['a'] + self.elev['b']*ds + self.elev['c']*(ds**2)+self.elev['d']*(ds**3))/self.scale
